...
def _classify_mask(mask: str) -> Tuple[int, str]:
    ...
    if is_nm and is_wc:                 # only 0.0.0.0 or 255.255.255.255
        return pc, "ambiguous"
    if is_nm:
        return pc, "netmask"
    if is_wc:
...
def _parse_port_op(tokens: List[str], i: int) -> Tuple[PortRange, int, bool]:
    """Parse an optional port operator; return (range, next_i, imprecise)."""
    if i >= len(tokens):
        return ANY_PORTS, i, False
    op = tokens[i]
    if op == "eq":
        p = _port_num(tokens[i + 1])
        return (PortRange(p, p) if p >= 0 else ANY_PORTS), i + 2, p < 0
    if op == "range":
        lo, hi = _port_num(tokens[i + 1]), _port_num(tokens[i + 2])
        if lo < 0 or hi < 0:
            return ANY_PORTS, i + 3, True
        return PortRange(min(lo, hi), max(lo, hi)), i + 3, False
    if op == "gt":
        p = _port_num(tokens[i + 1])
        if p < 0 or p >= PORT_MAX:
            return ANY_PORTS, i + 2, True
        return PortRange(p + 1, PORT_MAX), i + 2, False
    if op == "lt":
        p = _port_num(tokens[i + 1])
        if p <= PORT_MIN:
            return ANY_PORTS, i + 2, True
        return PortRange(PORT_MIN, p - 1), i + 2, False
    if op == "neq":
...
def _entry_tokens(line: str) -> List[str]:
    ...
    s = re.sub(r"(?i)^access-list\s+\S+\s+(?:extended\s+)?", "", s)  # ASA prefix
    return s.split()
...
def parse_acls(text: str) -> Tuple[List[ACE], List[str]]:
    ...
    for raw in text.splitlines():
        ...
        if not toks or toks[0].lower() not in _ACTIONS:
            ...
            continue
        try:
            ace, note = _parse_entry(toks, seq + 1, current_acl, stripped)
...
def _parse_entry(toks: List[str], seq: int, acl: str, raw: str):
    ...
    if ported:
        src_port, i, imp_sp = _parse_port_op(toks, i)
    else:
        src_port, imp_sp = ANY_PORTS, False
    dst, i, imp_d = _parse_addr(toks, i)
    if ported:
        dst_port, i, imp_dp = _parse_port_op(toks, i)
    else:
    ...
    note = None
    if (imp_s or imp_d):
        note = f"imprecise mask (treated conservatively): {raw}"
