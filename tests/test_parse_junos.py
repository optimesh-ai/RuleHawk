...
def test_unparseable_address_in_list_never_drops_member_silently():
    # J-ADDRS: an unparseable address inside a list must never just be dropped
    # (subset = invisible hole). Partial precision now emits the resolved
    # member as an EXACT ACE plus one opaque any/any imprecise ACE covering the
    # unresolved remainder — the true space stays inside the union of
    # (exact ACEs ∪ opaque marker), so a leak through the unresolved member can
    # never false-PASS, and the resolved member keeps its exact proof power.
    cfg = """
    firewall { family inet { filter F {
    ...
    assert len(precise) == 1 and str(precise[0].src) == "10.0.0.0/8"
    assert len(opaque) == 1
